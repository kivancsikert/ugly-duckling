...
public:
    ...
    virtual std::shared_ptr<Peripheral<TConfig>> createPeripheral(const std::string& name, const std::shared_ptr<TSettings>& settings, const std::shared_ptr<MqttRoot>& mqttRoot, const PeripheralServices& services) = 0;
